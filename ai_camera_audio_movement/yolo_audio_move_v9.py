...
Usage:
 ...
 - THERE IS A 5 MINUTE MAX FOR AUDIO DETECTION
"""
        ...
        with MicrophoneStream(RATE, CHUNK) as stream:
            ...
            try:
                ...
                if e.code() == grpc.StatusCode.OUT_OF_RANGE:
                    rospy.logwarn("🎧 Stream limit reached, restarting audio stream...")
                    continue  # restart the while loop to re-open stream
                else:
                    rospy.logerr(f"🎙️ Audio thread error: {e}")
                    break
